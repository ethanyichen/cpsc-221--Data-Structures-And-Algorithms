...
void BinaryTree<T>::printPaths() const {
    ...
    printPaths(root, myString);
}


template<typename T>
void BinaryTree<T>::printPaths(const Node *subRoot, string myString) const {
    if (subRoot != NULL) {
        string myNewString = myString + " " + to_string(subRoot->elem);
        if ((subRoot->left != NULL) || (subRoot->right != NULL)) {
            printPaths(subRoot->left, myNewString);
            printPaths(subRoot->right, myNewString);
        } else {
...
template<typename T>
int BinaryTree<T>::sumDistances(const Node *subRoot, int currDis) const {
    if (subRoot != NULL) {
        int sum = currDis;
        currDis++;
        sum = sum + sumDistances(subRoot->left, currDis) + sumDistances(subRoot->right, currDis);
        return sum;
    } else {
        return 0;
    }
...
bool BinaryTree<T>::isOrdered() const {
    Node * prev = NULL;
    return isOrdered(root, prev);
}

template<typename T>
bool BinaryTree<T>::isOrdered(const Node *subRoot, Node *prev) const {
    bool result = true;
    if (subRoot != NULL) {
        result = result && isOrdered(subRoot->left, prev);
        if (prev != NULL) {
            result = result && (prev->elem < subRoot->elem);
            prev->elem = subRoot->elem;
        } else{
            prev = new Node(subRoot->elem);
        }
        result = result && isOrdered(subRoot->right, prev);
        return result;
    }
    return result;
}
