...
class BinaryTree
{
  ...
  private:
    ...
    void printPaths(const Node *subRoot, string myString) const;

    int sumDistances(const Node *subRoot, int currDis) const;

    bool isOrdered(const Node *subRoot, Node* prev) const;
